...
<section class="flex items-center justify-between gap-4 flex-wrap mb-10">
  <div>
    <h1 class="text-4xl font-black text-base-content tracking-tight"><?= htmlspecialchars(t('portfolio.title_pre')) ?> <span class="text-rose-500"><?= htmlspecialchars(t('portfolio.title_span')) ?></span></h1>
    <p class="text-base-content/60 mt-1"><?= htmlspecialchars(t('portfolio.desc')) ?></p>
  </div>
  <div class="flex gap-2">
    <div class="form-control w-56">
      <label class="label"><span class="label-text font-bold text-rose-400"><?= htmlspecialchars(t('portfolio.search_label')) ?></span></label>
      <input id="gallerySearch" type="text" placeholder="<?= htmlspecialchars(t('portfolio.search_placeholder')) ?>" class="input input-bordered w-full rounded-xl border-rose-100 focus:border-rose-300" />
    </div>
  </div>
</section>
...
<section class="mt-6 glass-card p-8 rounded-[2.5rem]">
  <div class="flex gap-3 flex-wrap items-center">
    <div class="dropdown">
      <label tabindex="0" class="btn bg-white border-rose-100 text-rose-500 hover:bg-rose-50 rounded-xl"><?= htmlspecialchars(t('portfolio.category')) ?></label>
      <div tabindex="0" class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-64 z-[20]">
        <button class="menu-item text-left px-4 py-2 hover:bg-rose-50 rounded-xl block w-full font-medium" data-cat=""><?= htmlspecialchars(t('portfolio.all_categories')) ?></button>
    ...
    <div class="dropdown">
      <label tabindex="0" class="btn bg-white border-rose-100 text-rose-500 hover:bg-rose-50 rounded-xl"><?= htmlspecialchars(t('portfolio.album')) ?></label>
      <div tabindex="0" class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-64 z-[20]">
        <button class="menu-item text-left px-4 py-2 hover:bg-rose-50 rounded-xl block w-full font-medium" data-alb=""><?= htmlspecialchars(t('portfolio.all_albums')) ?></button>
    ...
    <button id="galleryReset" class="btn btn-sm btn-ghost text-base-content/40 hover:text-rose-500"><?= htmlspecialchars(t('portfolio.reset_filters')) ?></button>
  </div>
  ...
  <div class="mt-6 flex justify-center">
    <button id="galleryLoadMore" class="btn bg-gradient-to-r from-rose-400 to-pink-500 text-white border-none rounded-2xl px-10 shadow-lg shadow-rose-200"><?= htmlspecialchars(t('portfolio.load_more')) ?></button>
  </div>

  <div id="galleryEnd" class="mt-4 text-center text-base-content/60 hidden"><?= htmlspecialchars(t('portfolio.no_more')) ?></div>
</section>
