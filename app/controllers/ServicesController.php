...
final class ServicesController
{
    public function index(): string
    {
        ...
        $t = $isAr ? [
            'badge' => 'عروضنا',
            'title_h' => 'خدمات <span class="text-rose-500">بصرية</span> متميزة',
            'desc' => 'باقات تصوير مصممة خصيصاً لتحكي قصتك الفريدة بأناقة ودقة.',
            'book' => 'احجز هذه الجلسة'
        ] : [
            'badge' => 'Our Offerings',
            'title_h' => 'Premium <span class="text-rose-500">Visual</span> Services',
        ...
        $content = '<div class="text-center mb-16">'
            ...
            . '<h1 class="text-5xl font-black mt-4 tracking-tight">'. $t['title_h'] .'</h1>'
            . '<p class="mt-4 text-base-content/60 max-w-xl mx-auto">'. $t['desc'] .'</p>'
            . '</div>'
            . '<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">';

        if (empty($services)) {
            $content .= '<div class="col-span-full text-center py-20"><p class="text-base-content/50 italic">Services will be available soon.</p></div>';
        } else {
            foreach ($services as $s) {
                $content .= '<div class="glass-card p-10 rounded-[3rem] flex flex-col hover:scale-[1.02] transition-transform duration-500">'
                    ...
                    . '<a href="?r=/booking&service_id=' . $s['id'] . '" class="btn bg-base-content text-base-100 border-none rounded-2xl h-14">'. $t['book'] .'</a>'
                    . '</div>';
            }
        }
